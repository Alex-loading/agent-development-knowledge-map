...
**视觉教学已发布两个模块**：LLM 基础八课共接入 40 张主视觉，每课 1 张总览图与 4 张段落视觉；Agent Harness 八课共接入 24 张主视觉，对应 27 个 SVG 文件，其中额外 3 个分步状态属于同一工具轨迹主视觉。两者都通过统一 visual registry 解析本地静态资产、证据归属、替代文本、长描述、图注与分步状态。Agent 机制、上下文、RAG 与记忆、AI 后端工程尚未视觉化，目前仍以知识笔记作为站内主教材。
