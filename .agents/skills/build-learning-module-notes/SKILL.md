...
6. Build a cognitive visual inventory before storyboarding. For every candidate figure, first state the exact learner question it must answer, the assessed outcome it supports, the owning note section, and why a visual is better than prose. Decide `original-synthesis`, licensed use, or `link-only-original-replacement` under [source-policy.md](references/source-policy.md); do not start from search-image results.
7. Synthesize by theme, never by source order. Use [chapter-standard.md](references/chapter-standard.md) to move from prerequisite bridge and intuition through accurate mechanism, engineering meaning, concrete examples, misconceptions, recap, and the next lesson.
8. Attach valid `sourceIds` to every substantive section and published visual of a formal chapter. Use only real resource IDs backed by attributable evidence cards; never convert `lesson.explanations`, objectives, quiz, interview, exercise, completion criteria, or other course-field paths into source IDs. A visual's source IDs must be a subset of its owning section's source IDs and resolve through the lesson and project registries.
9. Produce data only, with no HTML, according to [data-contract.md](references/data-contract.md). Keep visuals in the shared visual registry and place them from notes with `overviewVisualId` or `sections[*].visuals[*]`; never embed SVG, image markup, or arbitrary renderer callbacks in lesson data. For a formal project chapter, validate every section and visual source ID. If evidence remains insufficient, return a blocked artifact rather than a contract-shaped publishable note.
10. Validate visual assets and rendering when the module declares visual completion: local assets only, safe inactive SVG, descriptive fallback, accessible long descriptions, responsive local media scrolling, reduced motion, and keyboard-safe step controls. A legacy module may omit visuals until it is explicitly migrated; after it declares visual completion, every visual gate is mandatory.
...
12. Score the result with [quality-rubric.md](references/quality-rubric.md). Do not publish below 85/100, with broken `sourceIds` or `visualId` references, or with a failed visual gate; revise the chapter or explicitly report the blocking gap.
...
- When visual completion is declared, every figure answers a recorded cognitive question, has exactly one evidence-owning placement, resolves through the visual registry, and passes the complete source, permission, safety, accessibility, responsive, fallback, and interaction gates.
...
| A storyboard starts with an attractive source image. | Start with the learner's cognitive question and assessed outcome, then choose the minimum useful visual form. |
| A third-party page is accessible, so its figure is downloaded. | Verify redistribution and, for adaptations, modification permission; otherwise record `link-only-original-replacement` and create an original visual from independently supported facts. |
| Visual references render, but evidence ownership is unclear. | Require one owning section and keep each visual's `sourceIds` within that section's verified source IDs. |
| Visuals are called optional after a module claims migration is complete. | Legacy omission is allowed only before declared visual completion; afterward all visual gates block publication. |
