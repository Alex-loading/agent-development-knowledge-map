  ...
  sections: [
    ...
    },
    {
      ...
      paragraphs: [
        ...
        '缓存击穿发生在热门 key 同时过期，大量请求并发回源。singleflight 是指在一个协调域内，同一 key 同时只允许一个请求执行回源，其余请求等待并共享该次结果；它可以与随机 TTL、提前刷新和有界回源并发组合。若协调跨进程，锁持有者可能暂停，仍需有效性 token 与超时；最重要的是保护权威数据库，不让所有请求无上限回源。',
      ...
      sourceIds: ['res-backend-redis-eviction', 'res-backend-postgres-transactions'],
    },
    {
      id: 'semantic-cache',
      title: '语义缓存先做硬隔离再谈相似',
