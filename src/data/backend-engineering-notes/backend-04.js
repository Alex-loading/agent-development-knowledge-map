...
export const backend04Note = deepFreeze({
  ...
  sections: [
    ...
    },
    {
      ...
      paragraphs: [
        ...
        '租约包含持有者、到期时间和递增 fencing token。worker 定期续租，只有持有最新 token 的尝试可以提交结果；若进程暂停过久，旧 worker 恢复后也不能覆盖新 worker。租约到期说明所有权可重新分配，不证明旧进程已经停止，因此外部副作用仍要使用幂等键或独立账本保护。',
      ...
      keyPoints: ['broker 确认时机不能自动创造 exactly-once', '租约和 fencing token 防止迟到 worker 覆盖新结果'],
  ...
  misconceptions: [
    ...
    { claim: '租约过期说明旧 worker 已停止。', correction: '旧进程可能只是暂停，必须用 fencing token 和副作用幂等阻止迟到提交。' },
  ...
  recap: [
    ...
    'broker ack、租约与 fencing token分别解决不同失败窗口。',
    '重试是带新 attempt、旧幂等身份和有限预算的受控动作。',
    '通过崩溃矩阵验证每个持久事件之间的恢复语义。',
  ],
  nextStep: '实现一个最小 job 表和 worker 协议：提交时持久化 job 与请求指纹，worker 以租约和递增 token 领取，完成时使用条件更新提交终态。为发布前、领取后、模型返回后、结果提交后和 ack 前五个位置注入进程终止，记录重投次数、最终状态和副作用计数。再加入队列深度与年龄告警，证明系统在 worker 重启后既不静默丢任务，也不会让迟到尝试覆盖新结果。最后演练一次有界死信重放并保留完整可查询审计记录。',
});
