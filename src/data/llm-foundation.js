...
const lessons = [
  {
    ...
    quiz: [
      ...
      quiz('quiz-llm-01-2', 'Agent 应用出现事实错误时，第一步更合理的做法是什么？', ['立刻从头训练模型', '增加所有上下文', '建立可复现样例并定位错误来源', '把温度固定为 1'], 2, '先用样例和评测判断是知识、检索、提示还是模型能力问题，才能选择成本合适的修复手段。'),
    ],
  ...
  },
  {
    ...
    quiz: [
      ...
      quiz('quiz-llm-02-2', '多层网络若完全没有非线性激活，主要会怎样？', ['一定无法计算梯度', '多层线性变换仍可合并为一个线性变换', '参数数量自动变成零', '损失函数不再需要'], 1, '线性变换的复合仍是线性变换，网络难以表达复杂的非线性关系。'),
    ],
  ...
  },
  {
    ...
    quiz: [
      ...
      quiz('quiz-llm-03-2', '上下文未超过窗口上限时，增加材料为何仍可能变差？', ['模型会自动删除参数', '无关或冲突信息可能干扰注意与指令执行', 'Embedding 会停止工作', '推理会变成训练'], 1, '窗口是容量约束而非质量保证；噪声、冲突和关键证据位置都会影响输出。'),
    ],
  ...
  },
  {
    ...
    quiz: [
      ...
      quiz('quiz-llm-04-2', 'decoder-only 语言模型使用因果掩码的主要原因是什么？', ['减少词表大小', '阻止训练位置读取未来 token', '删除所有残差连接', '让每个头参数相同'], 1, '下一 token 预测不能偷看答案，因果掩码让训练时的信息可见性符合生成过程。'),
    ],
  ...
  },
  {
    ...
    quiz: [
      ...
      quiz('quiz-llm-05-2', 'LoRA 的核心取舍更接近哪项？', ['完全不训练任何参数', '只训练低秩增量以降低微调资源成本', '自动消除幻觉', '把上下文窗口变成无限'], 1, 'LoRA 冻结大部分原参数并学习低秩适配参数，但仍需训练数据、验证与部署管理。'),
    ],
  ...
  },
  {
    ...
    quiz: [
      ...
      quiz('quiz-llm-06-2', 'KV Cache 加速自回归生成的原因是什么？', ['缓存历史 token 的 K/V，避免每步重复计算全部前缀', '提前存下所有正确答案', '减少模型参数数量', '取消注意力计算'], 0, '历史位置的 K/V 在后续步可复用，新 token 仍要计算并与缓存做注意力。'),
    ],
  ...
  },
  {
    ...
    quiz: [
      ...
      quiz('quiz-llm-07-2', '检索文档中出现“忽略之前规则”时，应如何理解？', ['它自动升级为系统指令', '它属于不可信数据，不能覆盖更高层规则', '它证明模型已被微调', '它应无限重试'], 1, '外部内容是任务数据而不是授权来源，应用要通过边界、权限和工具校验抵抗注入。'),
    ],
  ...
  },
  {
    ...
    quiz: [
      ...
      quiz('quiz-llm-08-2', '高质量离线评测集应该主要从哪里演进？', ['只用随机百科问题', '真实需求、边界条件与生产失败样例', '只保留模型答对的题', '每次发布都换成完全不同指标'], 1, '真实任务和失败日志让评测能预测用户风险，版本化固定集还能发现回归。'),
    ],
...
const interviewQuestions = [
  ...
  {
    id: 'iq-llm-01-2', lessonId: 'llm-01', question: '训练和推理的核心区别是什么？',
    shortAnswer: '训练通过数据、损失和优化器反复更新参数，目标是学到可泛化规律；推理固定参数，根据当前输入计算输出，关注质量、延迟、吞吐和成本。',
    deepDive: ['训练包含前向、损失、反向和更新，资源通常受计算与数据管线约束。', '推理可分 prefill 和 decode，应用还需管理缓存、并发、采样和失败重试。'],
  ...
  {
    id: 'iq-llm-02-1', lessonId: 'llm-02', question: '请解释神经网络一次参数更新。',
    shortAnswer: '输入经过带参数的层得到预测，损失函数度量预测和目标的差距，反向传播用链式法则得到参数梯度，优化器按学习率等规则更新参数，再用新批次重复。',
    deepDive: ['梯度表示当前点附近参数微小变化对损失的影响，不是全局最优方向保证。', '监控训练和验证损失可区分优化困难与泛化问题，还需观察梯度尺度和数据质量。'],
  ...
  {
    id: 'iq-llm-06-1', lessonId: 'llm-06', question: 'temperature 与 top-p 有什么区别？',
    shortAnswer: 'temperature 缩放 logits，整体改变概率分布的尖锐程度；top-p 按概率从高到低保留累计质量达到阈值的动态候选集合。两者都控制采样，不会增加知识或保证正确。',
    deepDive: ['低温使高分 token 更占优势，高温扩大低分候选影响；极端设置可能损害质量。', 'top-p 的候选数量随每一步分布变化，实践中应按任务通过重复试验选择参数。'],
