const sections = Object.freeze([
  Object.freeze({
    ...
    paragraphs: Object.freeze([
      '二十轮运行若只依赖聊天历史，下一轮既难找当前约束，也会把旧计划当现状。Transcript 保存用户/模型/工具原始消息，回答“当时说了什么”；append-only event log 保存动作、观察、状态迁移和控制决定，回答“系统怎样到这里”。两者可追溯，却不直接充当当前状态。',
      'Working state 是事件归约出的当前快照，如合同版本、步骤、预算与 pending；working memory 是单任务跨调用保留的事实、假设、计划、观察索引和指针；per-call context 是某次调用临时装入的有限视图。Artifact pointer 保存 ID、版本、类型与访问边界，不复制全文。',
      'CoALA 支撑跨模型调用维持感知、目标、中间信息并接回环境反馈；Weng 综述交叉说明短期/长期记忆与窗口问题。它们不规定生产字段；五层职责和 pointer 格式是课程综合。字段归属取决于是否须原样追溯、参与当前计算、跨调用保留及值得占用本轮 token。',
    ]),
  ...
  Object.freeze({
    ...
    paragraphs: Object.freeze([
      ...
      'Observation 至少带 observationId、sourceRef、observedAt、sourceVersion、integrityStatus 与证据指针；fact/belief 带 statement、version、derivedFrom、confidence、validUntil、validatorProvenance。Confidence 不能覆盖权限、有效期或验证结果；到期、来源版本变化或证据冲突会使条目 stale/contested。验证记录须含验证器 ID/版本、输入证据与 PASS/FAIL/UNKNOWN，不能只写“已检查”。',
