const sections = Object.freeze([
  ...
  Object.freeze({
    ...
    paragraphs: Object.freeze([
      ...
      '每个版本化步骤采用 pending → ready → running → passed/failed/unknown/invalidated：pending 等待依赖，依赖的当前版本均 passed 后才 ready；dispatch 必须以原子 claim 把 planVersion、stepVersion、attempt 与 stableIntent 绑定后才进入 running；完成回写使用 CAS（compare-and-swap）或 fencing token，只允许仍匹配当前 planVersion、stepVersion 和 attempt 的 claim 发布 artifact、满足 checkpoint 并解锁依赖。计划修订可把旧 pending、ready 或 running 标成 invalidated；旧 worker 的迟到成功只能追加 observation 供审计，不能发布 artifact 或改变新版本依赖。unknown 副作用按 stable intent 对账，未确认安全前不允许再次 act。这套并发和版本协议是课程工程综合，不是 Plan-and-Solve 或 ReWOO 的原论文机制。运行约束 JSON：{"states":["pending","ready","running","passed","failed","unknown","invalidated"],"claimKey":["planVersion","stepVersion","attempt","stableIntent"],"dispatch":"atomic-claim","completion":"cas-with-fencing","currentVersionPublishesArtifact":true,"staleCompletion":"append-observation-only","unknownSideEffect":"reconcile-by-stable-intent-no-repeat-act"}',
    ]),
    keyPoints: Object.freeze([
      ...
      '原子 claim 与完成 CAS/fencing 共同阻止旧 plan 或旧 attempt 的迟到结果发布 artifact、满足 checkpoint 或解锁新依赖。',
    ]),
    callout: Object.freeze({
      kind: 'boundary',
      title: '效率结果有实验边界',
