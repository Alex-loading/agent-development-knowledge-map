const sections = Object.freeze([
  ...
  Object.freeze({
    ...
    paragraphs: Object.freeze([
      ...
      '每个版本化步骤采用 pending → ready → running → passed/failed/unknown/invalidated：pending 等待依赖，依赖的当前版本均 passed 后才 ready；dispatch 必须以原子 claim 绑定完整 expected tuple，即 stepId、planVersion、stepVersion、attempt、stableIntent 与单调递增的 fencingToken，随后才进入 running。完成或 commit 使用 CAS 同时比较当前 planVersion、目标 stepId、stepVersion、attempt 和仍有效的 fencingToken；只有完整 tuple 匹配的当前 worker 能发布 artifact、满足 checkpoint 并解锁依赖。计划修订可把旧 pending、ready 或 running 标成 invalidated；来自旧 plan、旧 stepVersion、旧 attempt、错误 stepId 或失效 fencing token 的迟到成功都只能追加 observation 供审计，不能发布 artifact 或改变当前依赖。unknown 副作用按 stable intent 对账，未确认安全前不允许再次 act。这套并发和版本协议是课程工程综合，不是 Plan-and-Solve 或 ReWOO 的原论文机制。运行约束 JSON：{"states":["pending","ready","running","passed","failed","unknown","invalidated"],"claimKey":["stepId","planVersion","stepVersion","attempt","stableIntent","fencingToken"],"dispatch":"atomic-claim-expected-tuple","completion":"cas-current-tuple-with-valid-fencing","currentTuplePublishesArtifact":true,"staleCompletion":"append-observation-only","unknownSideEffect":"reconcile-by-stable-intent-no-repeat-act"}',
    ]),
    keyPoints: Object.freeze([
      ...
      '原子 claim 与完成 CAS/fencing 必须比较 stepId、planVersion、stepVersion、attempt 和有效 fencing token，阻止任何陈旧 tuple 发布 artifact、满足 checkpoint 或解锁依赖。',
    ]),
    callout: Object.freeze({
      kind: 'boundary',
      title: '效率结果有实验边界',
