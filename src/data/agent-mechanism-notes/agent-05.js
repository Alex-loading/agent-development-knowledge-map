const sections = Object.freeze([
  ...
  Object.freeze({
    ...
    paragraphs: Object.freeze([
      ...
      '合理粒度由工具能力、错误隔离和下游消费方式共同决定。若一个步骤同时要求发现候选、核验资质、比较报价并写建议，任一失败都无法定位，应该拆分；若十个步骤只是逐字段复制同一记录、使用同一工具且共享同一验收，则管理开销可能超过隔离收益，应合并。依赖图用有向边表示“产物 A 必须先于步骤 B”，可发现循环依赖与关键路径；每个下游步骤还要明确消费哪个 artifact 的哪个版本，防止计划看似有顺序、实际却没有数据连接。',
  ...
  Object.freeze({
    ...
    paragraphs: Object.freeze([
      '计划是一组基于当前 belief 的行动假设，不是执行命令的真理。每个 checkpoint 都要比较 expected observation 与 actual observation，并把差异分类为局部步骤失败、依赖失效、新约束、目标变化或证据不足。只有相关状态发生最小但实质的变化才触发修订；模型因为措辞不同就重写计划不算新证据。ReAct 能支撑环境 observation 进入后续决策，ReWOO 则明确展示固定蓝图在工具失败时的适应性限制；本段的触发分类和 planVersion 规则属于课程工程综合。',
      '局部修订适用于目标和大部分依赖仍有效的情况：替换一个数据源、修正一个查询、重跑受影响步骤，并使所有消费旧 artifact 的下游节点失效。整体重规划适用于顶层目标或硬约束改变、关键依赖永久不可用、依赖图产生冲突，或多次局部恢复仍无进展。任何修订都应保留旧 planVersion、触发 observation、受影响节点、保留节点、废弃理由和预算差异，使评审者能回答“为什么改、改了什么、旧产物还能否用”。',
      '为防止重规划震荡，控制器同时限制 replan count、总模型调用、墙钟时间与连续无进展次数，并可要求冷却或最小状态差异。若新计划只是给同一失败动作换个名字、没有新的 artifact 或 checkpoint，就应停止并 blocked/handoff，而不是继续烧预算。反过来，忠于旧计划也不是稳定性：新硬约束已明确冲突时继续执行，会把早期错误传播到所有下游步骤。稳定来自可解释的触发与版本化修订，不来自永不修改。',
    ]),
    keyPoints: Object.freeze([
  ...
  Object.freeze({
    ...
    paragraphs: Object.freeze([
      ...
      '策略棋盘 JSON：[ {"strategy":"reactive","actions":["search-candidates","inspect-next-gap","fetch-source","update-board"],"artifacts":["evidence-board","gap-list"],"dependencies":["每轮只依赖最新board与gap-list"],"checkpoints":["每次调用后校验来源、去重与新增证据"],"budgets":{"toolCalls":8,"replans":0,"wallClockMinutes":10}}, {"strategy":"plan-and-execute","actions":["build-plan","discover","verify-constraints","compare","draft-shortlist"],"artifacts":["plan-v1","candidate-table","constraint-report","shortlist"],"dependencies":["discover→verify-constraints→compare→draft-shortlist"],"checkpoints":["每个artifact通过schema与来源检查"],"budgets":{"toolCalls":8,"replans":1,"wallClockMinutes":10}}, {"strategy":"hybrid","actions":["set-milestones","react-within-milestone","verify","revise-affected-tail"],"artifacts":["milestone-plan","evidence-board","plan-revision-log","shortlist"],"dependencies":["里程碑顺序固定、里程碑内动作由最新observation选择"],"checkpoints":["里程碑出口核验约束覆盖与证据缺口"],"budgets":{"toolCalls":8,"replans":2,"wallClockMinutes":10}} ]',
  ...
  Object.freeze({
    ...
    paragraphs: Object.freeze([
      ...
      '恢复记录 JSON：[ {"injection":"empty-result","reason":"工具成功但当前查询范围没有候选","recovery":"mark-scope-searched→change-query-or-source→replace-discovery-step","exit":"allowed-sources-exhausted则blocked"}, {"injection":"timeout","reason":"响应缺失时外部终态可能未知，副作用不能按失败假定","recovery":"record-unknown→reconcile-by-stable-intent→仅确认未执行且预算允许时重放","exit":"无法对账则handoff"}, {"injection":"new-constraint","reason":"硬约束改变使旧计划假设和部分artifact失效","recovery":"version-constraint→invalidate-dependent-artifacts→local-revise-or-full-replan→revalidate","exit":"约束不可同时满足则blocked"} ]。供应商研究只使用只读查询，风险较低；这条 timeout 规则仍按更严格的通用控制边界书写，避免学习者把同一模式误用到创建订单、付款或发送消息等副作用动作。',
      '页面 plan-recovery 实验只是本地确定性教学状态机：它根据选定策略、observation 类型和重试计数展示 retry、switch-action、replace-step、replan 或 blocked，用来检查“不同观察映射到不同恢复动作”。它不调用真实模型、供应商数据源或论文实现，不测量 ReAct、Plan-and-Solve、ReWOO、ToT 的能力与成功率；它不是模型、论文方法或生产可靠性的评测，也不证明界面给出的选择适合生产。真实系统还需接入工具终态对账、幂等/审计、真实依赖图、预算持久化和业务验证器；本练习只验证控制语义。',
    ]),
    keyPoints: Object.freeze([
