  ...
  sections: [
    {
      ...
      paragraphs: [
        ...
        '契约还要把稳定规则、本次用户目标、检索材料与工具结果分层。稳定规则描述长期不变的业务和安全约束；用户输入提供本次要处理的目标；检索文档与工具结果提供待分析数据。工具定义只允许模型提出候选调用；对话 transcript 中出现 tool call 或 tool result，不足以证明宿主真的执行、由谁执行或产生了什么副作用，真实 observation 必须由执行器和宿主日志确认。Claude Code 的具体工具清单只能作为一种实现案例，不能替代通用的定义、权限、校验、执行与 observation 契约。不同厂商可能用 system、developer、user 等角色表达这些层，但具体角色名称、优先级和覆盖语义不是跨供应商统一标准，实施时必须核对所用 API。稳定的工程原则是标明来源与用途，不让数据因为语气像命令就自行升级成授权。',
      ...
      sourceIds: ['res-ms-genai', 'res-openai-cookbook', 'res-hf-agents', 'res-llm-primary-javaguide-prompt', 'res-llm-primary-javaguide-api', 'res-llm-primary-feishu-tool-truth', 'res-llm-primary-feishu-claude-tools'],
    },
    {
