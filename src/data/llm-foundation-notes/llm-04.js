  ...
  sections: [
    ...
    },
    {
      ...
      paragraphs: [
        ...
        '必须诚实记录界面边界：项目里的 normalizeAttention 与界面层可见位置筛选使用非负手工分数做教学归一化；当可见分数总和大于零时，按各分数占总和的比例线性分配，若全部可见分数都为零，则代码回退为均匀权重。它没有学习到的 Q/K 投影，也没有计算指数 softmax。真实注意力核心是 softmax(QKᵀ/√d_k + mask)，随后用所得权重聚合 Value，并在多头结构中拼接和投影。因此界面百分比适合验证相对分配和 mask 直觉，却不应与真实模型数值、梯度或最终输出声明为相同。',
