...
const resources = [
  { id: 'res-harness-openai-running', title: 'Running Agents', url: 'https://openai.github.io/openai-agents-python/running_agents/', source: 'OpenAI Agents SDK', language: '英文', type: '官方文档', difficulty: '入门', stage: 'Runner 基础', value: '说明 Runner、run 配置与执行方式，可用于辨认模型决策和宿主运行控制的职责边界。', verifiedAt: VERIFIED_AT },
  { id: 'res-harness-openai-hitl', title: 'Human-in-the-loop', url: 'https://openai.github.io/openai-agents-python/human_in_the_loop/', source: 'OpenAI Agents SDK', language: '英文', type: '官方文档', difficulty: '进阶', stage: '人工审批', value: '展示工具审批导致的暂停、保存与恢复流程，适合对照长时间 HITL 的状态设计。', verifiedAt: VERIFIED_AT },
  { id: 'res-harness-openai-tools', title: 'Tools', url: 'https://openai.github.io/openai-agents-python/tools/', source: 'OpenAI Agents SDK', language: '英文', type: '官方文档', difficulty: '入门到进阶', stage: '工具治理', value: '提供工具声明与调用接口的官方语义，用来检查注册表、参数校验和执行层的分工。', verifiedAt: VERIFIED_AT },
  { id: 'res-harness-openai-run-state', title: 'RunState API Reference', url: 'https://openai.github.io/openai-agents-python/ref/run_state/', source: 'OpenAI Agents SDK', language: '英文', type: 'API 参考', difficulty: '进阶', stage: '运行状态', value: '提供可序列化运行状态的接口参考，便于理解暂停后续跑需要保存哪些控制信息。', verifiedAt: VERIFIED_AT },
  { id: 'res-harness-langgraph-persistence', title: 'Persistence', url: 'https://docs.langchain.com/oss/python/langgraph/persistence', source: 'LangGraph', language: '英文', type: '官方文档', difficulty: '进阶', stage: 'Checkpoint', value: '说明线程、checkpoint 与持久化机制，可用于比较状态快照、历史和跨进程恢复。', verifiedAt: VERIFIED_AT },
  { id: 'res-harness-langgraph-interrupts', title: 'Interrupts', url: 'https://docs.langchain.com/oss/python/langgraph/interrupts', source: 'LangGraph', language: '英文', type: '官方文档', difficulty: '进阶', stage: '暂停恢复', value: '说明中断点和恢复输入的运行语义，适合作为审批 pause/resume 的实现参考。', verifiedAt: VERIFIED_AT },
  { id: 'res-harness-langgraph-fault-tolerance', title: 'Fault tolerance', url: 'https://docs.langchain.com/oss/python/langgraph/fault-tolerance', source: 'LangGraph', language: '英文', type: '官方文档', difficulty: '进阶', stage: '失败恢复', value: '展示持久化执行面对节点故障时的恢复思路，同时提醒副作用仍需应用层幂等。', verifiedAt: VERIFIED_AT },
  { id: 'res-harness-temporal-execution', title: 'Workflow Execution', url: 'https://docs.temporal.io/workflow-execution', source: 'Temporal', language: '英文', type: '官方文档', difficulty: '进阶', stage: '耐久执行', value: '介绍耐久工作流执行及其生命周期，帮助建立 run 与进程生命周期分离的认识。', verifiedAt: VERIFIED_AT },
  { id: 'res-harness-temporal-event', title: 'Workflow Execution Event', url: 'https://docs.temporal.io/workflow-execution/event', source: 'Temporal', language: '英文', type: '官方文档', difficulty: '进阶', stage: '事件历史', value: '解释工作流事件及历史记录，用于理解追加事件如何支撑重放、审计与故障诊断。', verifiedAt: VERIFIED_AT },
  { id: 'res-harness-temporal-retry', title: 'Retry Policies', url: 'https://docs.temporal.io/encyclopedia/retry-policies', source: 'Temporal', language: '英文', type: '官方文档', difficulty: '进阶', stage: '重试策略', value: '说明重试策略参数与适用边界，可帮助区分可重试瞬态故障和业务终态错误。', verifiedAt: VERIFIED_AT },
  { id: 'res-harness-azure-durable', title: 'Durable Functions programming model overview', url: 'https://learn.microsoft.com/en-us/azure/azure-functions/durable/programming-model-overview', source: 'Microsoft Learn', language: '英文', type: '官方文档', difficulty: '进阶', stage: '耐久编排', value: '介绍 orchestrator、activity 与 durable state 的编程模型，用于比较确定性编排和 Agent Runner。', verifiedAt: VERIFIED_AT },
  { id: 'res-harness-aws-idempotent', title: 'Making retries safe with idempotent APIs', url: 'https://aws.amazon.com/builders-library/making-retries-safe-with-idempotent-APIs/', source: 'Amazon Builders’ Library', language: '英文', type: '工程文章', difficulty: '进阶', stage: '幂等设计', value: '系统讲解幂等请求标识和重复语义，是恢复副作用操作时避免重复写入的重要工程参考。', verifiedAt: VERIFIED_AT },
  { id: 'res-harness-aws-timeouts', title: 'Timeouts, retries, and backoff with jitter', url: 'https://aws.amazon.com/builders-library/timeouts-retries-and-backoff-with-jitter/', source: 'Amazon Builders’ Library', language: '英文', type: '工程文章', difficulty: '进阶', stage: '超时与退避', value: '解释 timeout、有限重试、退避和 jitter 的组合，帮助避免重试放大下游压力。', verifiedAt: VERIFIED_AT },
  { id: 'res-harness-sre-cascading', title: 'Addressing Cascading Failures', url: 'https://sre.google/sre-book/addressing-cascading-failures/', source: 'Google SRE', language: '英文', type: '工程书籍', difficulty: '进阶', stage: '故障控制', value: '从级联故障角度解释限流、超时与降级，适合校验 Harness 的预算和恢复策略。', verifiedAt: VERIFIED_AT },
  { id: 'res-harness-sre-overload', title: 'Handling Overload', url: 'https://sre.google/sre-book/handling-overload/', source: 'Google SRE', language: '英文', type: '工程书籍', difficulty: '进阶', stage: '背压', value: '说明过载保护、请求拒绝与负载治理，为队列容量和背压策略提供系统视角。', verifiedAt: VERIFIED_AT },
  { id: 'res-harness-gvisor', title: 'What is gVisor?', url: 'https://gvisor.dev/docs/architecture_guide/intro/', source: 'gVisor', language: '英文', type: '官方文档', difficulty: '进阶', stage: 'Sandbox 架构', value: '介绍应用内核隔离架构与安全边界，可用于比较容器、用户态内核和虚拟机方案。', verifiedAt: VERIFIED_AT },
  { id: 'res-harness-docker-seccomp', title: 'Seccomp security profiles for Docker', url: 'https://docs.docker.com/engine/security/seccomp/', source: 'Docker', language: '英文', type: '官方文档', difficulty: '进阶', stage: '系统调用隔离', value: '说明 seccomp 如何限制系统调用，帮助把最小权限落实到运行时内核接口层。', verifiedAt: VERIFIED_AT },
  { id: 'res-harness-docker-resources', title: 'Resource constraints', url: 'https://docs.docker.com/engine/containers/resource_constraints/', source: 'Docker', language: '英文', type: '官方文档', difficulty: '入门到进阶', stage: '资源预算', value: '介绍内存、CPU 等容器资源限制，可用于设计代码执行任务的硬资源上限。', verifiedAt: VERIFIED_AT },
  { id: 'res-harness-docker-rootless', title: 'Rootless mode', url: 'https://docs.docker.com/engine/security/rootless/', source: 'Docker', language: '英文', type: '官方文档', difficulty: '进阶', stage: '权限隔离', value: '介绍非 root 守护进程和容器的运行方式，是缩小宿主权限面的一个实践参考。', verifiedAt: VERIFIED_AT },
  { id: 'res-harness-firecracker', title: 'Firecracker design', url: 'https://github.com/firecracker-microvm/firecracker/blob/main/docs/design.md', source: 'Firecracker', language: '英文', type: 'GitHub 设计文档', difficulty: '深挖', stage: 'MicroVM 隔离', value: '描述 microVM 的设计目标和设备模型，可用于评估更强隔离带来的启动与运维权衡。', verifiedAt: VERIFIED_AT },
  { id: 'res-harness-nist-tool-use', title: 'Lessons Learned from a Consortium on Tool Use in Agent Systems', url: 'https://www.nist.gov/news-events/news/2025/08/lessons-learned-consortium-tool-use-agent-systems', source: 'NIST', language: '英文', type: '机构文章', difficulty: '进阶', stage: '工具风险', value: '从机构视角讨论 Agent 工具使用中的身份、授权与治理问题，适合补充宿主控制面设计。', verifiedAt: VERIFIED_AT },
  { id: 'res-harness-owasp-agency', title: 'LLM06:2025 Excessive Agency', url: 'https://genai.owasp.org/llmrisk/llm062025-excessive-agency/', source: 'OWASP GenAI', language: '英文', type: '安全指南', difficulty: '进阶', stage: '权限治理', value: '归纳过度功能、权限和自治带来的风险，为最小权限、审批和可撤销能力提供检查清单。', verifiedAt: VERIFIED_AT },
  { id: 'res-harness-agent-learning-hub', title: 'Agent-Learning-Hub', url: 'https://github.com/datawhalechina/Agent-Learning-Hub', source: 'Datawhale', language: '中文', type: 'GitHub 社区课程', difficulty: '入门到进阶', stage: '综合学习', value: '中文社区课程索引，可作为 Harness、框架与应用实践的扩展学习入口。', verifiedAt: VERIFIED_AT },
  { id: 'res-harness-agentscope-runtime', title: 'AgentScope Runtime', url: 'https://github.com/agentscope-ai/agentscope-runtime', source: 'AgentScope', language: '中英文', type: 'GitHub 开源项目', difficulty: '进阶', stage: '运行时实践', value: '开源 Agent 运行时提供部署、隔离与状态管理实现，可用于把课程概念映射到代码结构。', verifiedAt: VERIFIED_AT },
  { id: 'res-harness-smolagents-code', title: 'Code agents', url: 'https://huggingface.co/learn/agents-course/zh-CN/unit2/smolagents/code_agents', source: 'Hugging Face', language: '中文', type: '官方课程', difficulty: '入门到进阶', stage: '代码 Agent', value: '以代码 Agent 为例介绍执行方式，适合结合本课补充宿主验证与隔离要求。', verifiedAt: VERIFIED_AT },
  { id: 'res-harness-hello-agents-framework', title: 'Hello-Agents 第六章：框架开发实践', url: 'https://github.com/datawhalechina/hello-agents/blob/main/docs/chapter6/%E7%AC%AC%E5%85%AD%E7%AB%A0%20%E6%A1%86%E6%9E%B6%E5%BC%80%E5%8F%91%E5%AE%9E%E8%B7%B5.md', source: 'Datawhale', language: '中文', type: 'GitHub 社区教材', difficulty: '进阶', stage: '框架实践', value: '中文框架开发章节可用于对照 Runner、工具与上下文等抽象在代码中的组织方式。', verifiedAt: VERIFIED_AT },
  { id: 'res-harness-bilibili', title: 'Agent Harness 与运行时工程实践视频', url: 'https://www.bilibili.com/video/BV1HfHgzuEPn/', source: 'Bilibili 创作者', language: '中文', type: '社区视频', difficulty: '入门', stage: '直观补充', value: '用于建立运行时工程的直观认识，仅作实践补充，不作为安全或可靠性事实的唯一依据。', platform: 'Bilibili', verifiedAt: VERIFIED_AT },
  { id: 'res-harness-douyin', title: 'Agent 工程与 Harness 实作短视频', url: 'https://jingxuan.douyin.com/m/video/7646732508339457334', source: '抖音精选创作者', language: '中文', type: '社区视频', difficulty: '入门', stage: '实作补充', value: '提供中文短视频形式的实现观察，仅用于扩展案例，不替代官方文档、威胁模型和独立验证。', platform: '抖音', verifiedAt: VERIFIED_AT },
];
...
const lessons = [
  lesson({
    ...
    explanations: [
      ...
      { heading: 'Runner 生命周期必须可观测', body: '一个 run 应从 created、queued、running 出发，按事件进入 paused、blocked、completed、failed 或 cancelled 等状态。Runner 在开始、模型决策、工具调用、状态提交和终止处触发窄而稳定的 hooks，同时生成事件、指标、checkpoint 与产物引用，避免把业务逻辑隐藏在无法重放的回调里。', keyPoints: ['状态转换要由明确事件驱动并拒绝非法跳转', 'Hook 观察和扩展生命周期，但不得绕过核心校验'] },
    ],
  ...
  lesson({
    ...
    quizzes: [
      quiz('副作用成功但 checkpoint 失败后应怎样恢复？', ['直接生成新幂等键再调用', '先按原幂等键或远端 ID 查询并补写结果', '把 run 标记 completed'], 1, '旧 checkpoint 不证明副作用未发生；恢复必须先对账，确认后再推进本地状态。'),
  ...
  lesson({
    ...
    explanations: [
      { heading: '终态表达下一位操作者能做什么', body: 'Blocked 表示当前缺少权限、信息、依赖或人工决定，但满足解除条件后仍可继续；failed 表示本次尝试遇到不可恢复错误或已耗尽规定恢复策略；cancelled 表示收到停止意图并完成必要清理。人工审批则常进入 paused 而非终态，保存审批请求、过期时间和恢复令牌后释放 worker。', keyPoints: ['状态名必须配套原因码、证据和允许的后续动作', '长时间等待不能占用 worker、锁或短期凭证'] },
    ...
    exercise: { title: '制作可接管 handoff bundle', brief: '把一个等待生产发布审批且已有测试产物的 run 交给下一班工程师。', steps: ['判断应使用 paused、blocked、failed 还是 cancelled，并记录原因、解除条件、恢复令牌与清理状态', '整理目标、事件摘要、副作用账本、预算、审批、产物哈希、验证证据和下一步，检查敏感信息处理'], deliverable: '一个可机读 handoff bundle、产物 manifest 和人工接管检查表。' },
    quizzes: [
      quiz('缺少用户提供的生产账号但稍后可继续，应是什么状态？', ['failed', 'blocked', 'completed'], 1, '缺少可补充依赖且存在明确解除条件时属于 blocked，不应伪装为失败或完成。'),
...
const interviewSpecs = [
  ...
  [1, '请设计一个 Agent run 的生命周期状态机。', '判断标准是状态、事件和守卫完整：created、queued、running、paused、blocked、completed、failed、cancelled 均有合法入口、终止性质和恢复规则。', ['每次转换应持久化原因、版本和触发者，并用幂等命令避免重复转换。', 'paused 用于可恢复等待，blocked 表达解除条件，completed 必须绑定验收证据。'], '只画 running 到 success 或 error 两条线，没有取消、审批和恢复路径。', '审批过期和取消同时发生时如何决定转换顺序？', '高', '进阶', ['Agent 开发', '后端工程']],
  ...
  [8, '如何实现长时间人工审批的 pause/resume？', '判断标准是审批请求持久化后 run 进入 paused，释放 worker 和短期资源；回调验证令牌、期限、调用摘要和权限后由新 worker 续跑。', ['审批通知与状态提交需防重复，重复批准不能执行两次副作用。', '恢复时重新获取依赖、凭证和租约，并校验工具及策略版本。'], '让 worker 持有数据库连接和访问令牌阻塞等待几天，收到消息后直接执行。', '审批被拒、过期或修改参数时各自进入什么状态？', '高', '进阶', ['Agent 开发', 'AI 应用', '后端工程']],
...
export const agentHarness = deepFreeze({
  id: 'agent-harness',
  title: 'Agent Harness',
  summary: '学习如何用宿主 Runner、耐久状态、权限隔离、预算恢复、队列背压和人工接管，把 Agent 决策变成可治理的生产运行。',
  lessons,
  resources,
  interviewQuestions,
});
