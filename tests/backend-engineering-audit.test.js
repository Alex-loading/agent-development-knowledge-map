...
test('backend reconstruction audit reports exact production parity and rubric gates', async () => {
  ...
  const cards = backendEngineeringScenes.flatMap(({ cards: values }) => values);
  ...
  assert.match(audit, /视觉评分\s*\|\s*56\s*\/\s*60/);
  ...
  assert.match(audit, /浏览器.*未执行|browser.*not executed/is);
});
