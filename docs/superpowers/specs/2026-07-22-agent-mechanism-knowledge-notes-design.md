...
LLM 基础八课已经验证“站内知识笔记是主教材、外部资料是证据与扩展”的学习模式。设计立项时，第二模块 Agent 机制拥有完整的八课骨架、28 份基线资料、24 道面试题和三个交互实验，但每课仍只有两段 `explanations`，学习者必须自行拼接论文、课程、厂商文章和视频，难以形成从任务契约到可终止控制循环的完整知识体系。
...
## 设计修订 / Amendment — 2026-07-22

Task 5 的规格审查发现，`agent-03` 把幂等请求标识、网络超时后的未知终态和安全重试作为核心考核内容，但原始 28 份基线资料没有一份可访问正文直接支撑这组可靠执行语义。课程 objectives、exercise、quiz 和面试字段只能定义“要教什么”，不能充当“该机制为何成立”的外部证据；因此不能靠课程字段、现有资源标题或课程综合补齐这个证据缺口。

为满足“关键 assessed outcome 必须有可访问核心证据”的发布门槛，本修订对“不新增资源总数”作一次明确例外，新增第 29 份 official/core 来源：`res-agent-aws-idempotent-apis`，标题 `Making retries safe with idempotent APIs`，canonical URL 为 `https://aws.amazon.com/builders-library/making-retries-safe-with-idempotent-APIs/`。该来源只直接支撑调用方请求标识、幂等重试、网络超时不确定性、语义等价响应与晚到请求边界；订单审批、查询、对账和补偿仍属于课程综合或具体系统设计。

本修订只覆盖资源数量与上述证据缺口，不改变八课范围、UI、课程字段、实验逻辑或其他非目标。此后的最终 registry、测试、内容审计、集成、PR 与发布门槛均按“28 份原始基线 + 1 份 AWS 审查补充 = 29 份”执行；文档中保留的 28 份与“不增总数”描述是原始设计基线，不应被回写成事后已有 29 份。
...
制作过程必须使用 `.agents/skills/build-learning-module-notes/`。课程字段只定义学习覆盖，不构成来源证据。原始 28 个基线资源逐一建立 evidence card；按 2026-07-22 Amendment 新增的 AWS 资源使用同一 evidence contract，最终共 29 张：
...
实现先建立 RED 测试和 28 张基线来源卡，再按课程顺序写作；Task 5 规格审查触发 2026-07-22 Amendment 后，补入第 29 张 AWS 来源卡并更新最终门槛。每个课程任务使用独立作者；作者完成、自测和自审后，未参与写作的代理先做规格合规审查，再做内容质量审查。任何规格缺口由原作者修正并重新规格审查；只有规格通过后才进入质量审查，质量问题修复后必须复审。
...
- 按 2026-07-22 Amendment，最终 29/29 资源拥有合法 evidence，时敏日期使用真实核验值；
...
内容审计分别记录 28 份基线资料和 1 份 AWS 审查补充的访问类型、角色、增加原因与限制，以及每课 coverage matrix、质量量表、断裂引用和测试结果。单课发布门槛为 85/100；最终目标为无课程字段来源伪装、`brokenReferenceCount === 0`、所有八课均达到门槛。
