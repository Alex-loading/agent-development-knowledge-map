...
| `agent-04` | objectives[0..1] 与 concepts[0..4] → `state-read-and-termination-priority`、`decide-from-observable-state`、`validate-and-act-boundary`、`observe-and-update-state`、`progress-and-repetition-detection`、`react-boundary-and-observable-logging` | quiz[0] → `observe-and-update-state`；quiz[1] → `progress-and-repetition-detection` | `iq-agent-04-1` short/deepDive/misconception/followUp → 第 1/3/4/5/7 节；`iq-agent-04-2` 全字段 → 第 2/6 节；`iq-agent-04-3` 全字段 → 第 4/5/7 节 | exercise.steps[0..1]、deliverable → `minimal-loop-and-deterministic-lab`；completionCriteria[0] → 第 1–5/7 节，completionCriteria[1] → 第 2/6 节 | 七节按状态读取与 failed-first 终止→候选分流→call-tool 的 validate/act→observe/update→进展指纹→ReAct 边界→可恢复伪代码递进；finish/handoff/fail/ask-user 均有宿主验证或状态写入，未决副作用先按原 intent 对账，只有 call-tool 越过副作用边界 | ReAct 原论文核心支撑交错 reasoning/action/observation 与四类实验边界；OpenAI/Anthropic 支撑 run、环境 ground truth、最大迭代与人工介入；AgentBench 支撑其评测中的 invalid action、TLE、重复和终止失败；Lilian Weng 只作 ReAct/重复轨迹综述交叉核验；Berkeley 与 metadata-only Bilibili 不作机制来源 | 98（25/20/25/19/9） | ready-for-integration |
...
| 下方“`agent-04` 可复制 contract/source/freeze probe”完整 one-liner | 0 | 37 分钟；7 个唯一 section ID；正文 8529 字符且不超过 9000；每节 3 段、每段最短 183 字符、至少 2 个要点、非空且无重复 source 数组；6 个误区、11 个 recap；所有嵌套值冻结；5 个实质来源均通过全局 registry、lesson resourceIds 与 evidence set 三重解析，`brokenReferenceCount: 0` |
| 下方“`agent-04` 控制语义 probe”完整 one-liner | 0 | 除 failed-first、五类 candidate、pending/幂等持久化、工具顺序与 state/version 指纹外，额外断言 unknown recovery 原子转为 requires-human、关闭 auto reconcile、保留 evidenceRef 并直接 handoff；该分支无 continue/act，完整循环只出现一次 `reconcile_pending`，小状态机结果为一次 reconcile、零次 act；全部通过 |
...
本轮输出：`{"readingMinutes":37,"body":8529,"sections":7,"minParagraph":183,"sources":["res-agent-anthropic-effective","res-agent-openai-guide","res-agent-agentbench","res-agent-react-paper","res-agent-lilian-weng"],"misconceptions":6,"recap":11,"deepFrozen":true,"brokenReferenceCount":0}`。

### `agent-04` 可复制控制语义 probe

以下命令直接检查正文中的可执行控制语义，而不是只数结构字段：它要求 failed 优先于 done，fatal 与 goal 同时成立时仍失败；五类 candidate 必须各自分流，只有 call-tool 进入 validate/act；副作用前必须创建稳定 intent 并持久化 pending action 与幂等映射；重启先 reconcile 原 intent 且禁止重复副作用；unknown outcome 必须原子隔离为 requires-human、关闭自动对账、保留证据并直接 handoff，不能 continue 或 act；工具路径必须维持 decide→validate→act→observe→update；相关 state/version 改变时必须形成新的完整动作指纹：

```bash
node --input-type=module -e 'import assert from "node:assert/strict"; const {agent04Note:n}=await import("./src/data/agent-mechanism-notes/agent-04.js"); const body=[n.introduction,...n.sections.flatMap(s=>s.paragraphs),n.nextStep].reduce((a,s)=>a+s.trim().length,0); const loop=n.sections.find(s=>s.id==="minimal-loop-and-deterministic-lab").paragraphs.join(" "); const repeat=n.sections.find(s=>s.id==="progress-and-repetition-detection").paragraphs.join(" "); for (const token of ["reconcile_pending", "reuse_original_intent", "do_not_repeat_side_effect", "finish-candidate", "verify_completion", "handoff-candidate", "record_handoff_request", "fail-candidate", "verify_unrecoverable_failure", "ask-user", "record_blocker_and_request", "call-tool", "get_or_create_intent(task, candidate)", "persist_pending_action", "idempotency_mapping", "validate(candidate", "act(candidate", "normalize_observation", "update_state"]) assert.ok(loop.includes(token), token); assert.match(loop,/priority=\[failed, done, handoff, blocked, budget-exhausted\]/); assert.equal(loop.match(/reconcile_pending/g).length,1); const unknown=loop.slice(loop.indexOf("if (recovery.unknown)"),loop.indexOf("view = read_state_view")); for(const token of ["mark_pending_requires_human","auto_reconcile: false","evidenceRef: recovery.evidenceRef","record_handoff_request","return terminal_result(\"handoff\""]) assert.ok(unknown.includes(token),token); assert.doesNotMatch(unknown,/continue|act\(/); let reconcileCount=0,actCount=0; const simulateUnknown=()=>{reconcileCount+=1;const recovery={unknown:true,evidenceRef:"ev-1"};if(recovery.unknown)return {status:"handoff",evidenceRef:recovery.evidenceRef};actCount+=1}; const simulated=simulateUnknown();assert.deepEqual(simulated,{status:"handoff",evidenceRef:"ev-1"});assert.equal(reconcileCount,1);assert.equal(actCount,0); assert.ok(loop.indexOf("candidate = decide") < loop.indexOf("validation = validate") && loop.indexOf("validation = validate") < loop.indexOf("raw = act") && loop.indexOf("raw = act") < loop.indexOf("obs = normalize_observation") && loop.indexOf("obs = normalize_observation") < loop.lastIndexOf("next = update_state")); assert.match(repeat,/state\/version 改变，完整动作指纹随之改变/); assert.doesNotMatch(repeat,/相同完整指纹/); assert.match(loop,/fatal 与 goalSatisfied 同时为 true 时返回 failed/); assert.ok(body>=4500&&body<=9000); console.log(JSON.stringify({body,failedBeforeDone:true,candidateBranches:true,completionVerified:true,pendingPersisted:true,reconcileBeforeDecision:loop.indexOf("reconcile_pending")<loop.indexOf("candidate = decide"),unknownRequiresHuman:true,autoReconcileDisabled:true,evidencePreserved:true,directHandoff:true,reconcileCount,actCount,toolOrder:true,stateVersionChangesFingerprint:true,fatalOverridesGoal:true}))'
```

本轮输出：`{"body":8529,"failedBeforeDone":true,"candidateBranches":true,"completionVerified":true,"pendingPersisted":true,"reconcileBeforeDecision":true,"unknownRequiresHuman":true,"autoReconcileDisabled":true,"evidencePreserved":true,"directHandoff":true,"reconcileCount":1,"actCount":0,"toolOrder":true,"stateVersionChangesFingerprint":true,"fatalOverridesGoal":true}`。
...
- `sections → evidence → 产出`：ReAct 原论文只支撑其设定中的 reasoning/action/observation 交错、环境交互和四类实验；Lilian Weng 只作二手综述与重复轨迹交叉核验；OpenAI/Anthropic 正文支撑 run、外部 ground truth、最大迭代、失败阈值与人工介入；AgentBench 只支撑其八环境、29 模型评测中的 invalid action、Task Limit Exceeded、重复和终止失败。课程另行综合确定 failed→done→handoff→blocked→budget-exhausted、五类 candidate 分流、重启 reconcile、执行前 pending/幂等映射持久化、call-tool 的完整 decide→validate→act→observe→update、state diff、四字段指纹与阈值；没有把这些结构反向伪装成任一论文或厂商标准。Berkeley 课程导航和 metadata-only Bilibili 均未出现在实质章节引用。
...
- `质量自评`：覆盖 25/25（两个 objectives、五 concepts、两题 quiz、三题面试全字段、exercise 全步骤/deliverable 与两个 completion criteria 均可追踪）；结构 20/20（七节从 agent-03 单次工具契约推进到控制循环并连接 agent-05 规划）；来源 25/25（五个实质来源均实际重访、三重解析且实验/版本限制清楚，两个 extension 不支撑机制）；教学 19/20（伪代码可照抄，五类 candidate 分流、完成谓词核验、failed-first 冲突处理、pending/reconcile 恢复语义、完整工具因果链、重复指纹与进展阈值均可操作，仍待集成页面验收）；数据契约 9/10（正文 8529 字符且不超过 9000、纯数据、稳定 kebab-case ID、无 HTML、深层冻结，最终对象身份由聚合测试验收）。总分 `98/100`，`brokenReferenceCount: 0`；coverage gap：没有阻塞性缺口，真实系统的完成谓词、fatal 不变量、handoff 政策、预算与阈值仍需按业务设计；状态 `ready-for-integration`，全模块仍为 `publicationReady: false`。
