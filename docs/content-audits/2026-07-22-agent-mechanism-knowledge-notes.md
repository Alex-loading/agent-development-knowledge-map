...
| 下方“`agent-04` 可复制 contract/source/freeze probe”完整 one-liner | 0 | 37 分钟；7 个唯一 section ID；正文 8187 字符且不超过 9000；每节 3 段、每段最短 183 字符、至少 2 个要点、非空且无重复 source 数组；6 个误区、11 个 recap；所有嵌套值冻结；5 个实质来源均通过全局 registry、lesson resourceIds 与 evidence set 三重解析，`brokenReferenceCount: 0` |
| 下方“`agent-04` 控制语义 probe”完整 one-liner | 0 | 断言 failed-before-done、fatal+goal→failed、五类 candidate 分流、finish completion verification、pending action/幂等映射先持久化、循环开头按原 intent reconcile 且不重复副作用、call-tool 的 decide→validate→act→observe→update 顺序，以及 state/version 变化生成新指纹；全部通过 |
...
本轮输出：`{"readingMinutes":37,"body":8187,"sections":7,"minParagraph":183,"sources":["res-agent-anthropic-effective","res-agent-openai-guide","res-agent-agentbench","res-agent-react-paper","res-agent-lilian-weng"],"misconceptions":6,"recap":11,"deepFrozen":true,"brokenReferenceCount":0}`。

### `agent-04` 可复制控制语义 probe

以下命令直接检查正文中的可执行控制语义，而不是只数结构字段：它要求 failed 优先于 done，fatal 与 goal 同时成立时仍失败；五类 candidate 必须各自分流，只有 call-tool 进入 validate/act；副作用前必须创建稳定 intent 并持久化 pending action 与幂等映射；重启先 reconcile 原 intent 且禁止重复副作用；工具路径必须维持 decide→validate→act→observe→update；相关 state/version 改变时必须形成新的完整动作指纹：

```bash
node --input-type=module -e 'import assert from "node:assert/strict"; const {agent04Note:n}=await import("./src/data/agent-mechanism-notes/agent-04.js"); const body=[n.introduction,...n.sections.flatMap(s=>s.paragraphs),n.nextStep].reduce((a,s)=>a+s.trim().length,0); const loop=n.sections.find(s=>s.id==="minimal-loop-and-deterministic-lab").paragraphs.join(" "); const repeat=n.sections.find(s=>s.id==="progress-and-repetition-detection").paragraphs.join(" "); for (const token of ["reconcile_pending", "reuse_original_intent", "do_not_repeat_side_effect", "finish-candidate", "verify_completion", "handoff-candidate", "record_handoff_request", "fail-candidate", "verify_unrecoverable_failure", "ask-user", "record_blocker_and_request", "call-tool", "get_or_create_intent(task, candidate)", "persist_pending_action", "idempotency_mapping", "validate(candidate", "act(candidate", "normalize_observation", "update_state"]) assert.ok(loop.includes(token), token); assert.match(loop,/priority=\[failed, done, handoff, blocked, budget-exhausted\]/); assert.ok(loop.indexOf("candidate = decide") < loop.indexOf("validation = validate") && loop.indexOf("validation = validate") < loop.indexOf("raw = act") && loop.indexOf("raw = act") < loop.indexOf("obs = normalize_observation") && loop.indexOf("obs = normalize_observation") < loop.lastIndexOf("next = update_state")); assert.match(repeat,/state\/version 改变，完整动作指纹随之改变/); assert.doesNotMatch(repeat,/相同完整指纹/); assert.match(loop,/fatal 与 goalSatisfied 同时为 true 时返回 failed/); assert.ok(body>=4500&&body<=9000); console.log(JSON.stringify({body,failedBeforeDone:true,candidateBranches:true,completionVerified:true,pendingPersisted:true,reconcileBeforeDecision:loop.indexOf("reconcile_pending")<loop.indexOf("candidate = decide"),toolOrder:true,stateVersionChangesFingerprint:true,fatalOverridesGoal:true}))'
```

本轮输出：`{"body":8187,"failedBeforeDone":true,"candidateBranches":true,"completionVerified":true,"pendingPersisted":true,"reconcileBeforeDecision":true,"toolOrder":true,"stateVersionChangesFingerprint":true,"fatalOverridesGoal":true}`。
...
- `质量自评`：覆盖 25/25（两个 objectives、五 concepts、两题 quiz、三题面试全字段、exercise 全步骤/deliverable 与两个 completion criteria 均可追踪）；结构 20/20（七节从 agent-03 单次工具契约推进到控制循环并连接 agent-05 规划）；来源 25/25（五个实质来源均实际重访、三重解析且实验/版本限制清楚，两个 extension 不支撑机制）；教学 19/20（伪代码可照抄，五类 candidate 分流、完成谓词核验、failed-first 冲突处理、pending/reconcile 恢复语义、完整工具因果链、重复指纹与进展阈值均可操作，仍待集成页面验收）；数据契约 9/10（正文 8187 字符且不超过 9000、纯数据、稳定 kebab-case ID、无 HTML、深层冻结，最终对象身份由聚合测试验收）。总分 `98/100`，`brokenReferenceCount: 0`；coverage gap：没有阻塞性缺口，真实系统的完成谓词、fatal 不变量、handoff 政策、预算与阈值仍需按业务设计；状态 `ready-for-integration`，全模块仍为 `publicationReady: false`。
